...
async fn malformed_existing_rows_fail_the_export_closed() -> anyhow::Result<()> {
    ...
    assert!(
        msg_b.contains("chunk.document"),
        "库 B 该报 chunk.document: {msg_b}"
    );

    // 逐页校验同样fail-closed：体检之后的 TOCTOU 坏行也不能漏出伪造 IRI
    assert!(
        utopia_store::export::evidence_page(&mut pool.begin().await?, f.a, None)
            .await
            .is_err()
    );
    assert!(
        utopia_store::export::evidence_page(&mut pool.begin().await?, f.b, None)
            .await
            .is_err()
    );
    assert!(
        utopia_store::export::chunks_page(&mut pool.begin().await?, f.b, None)
            .await
            .is_err()
    );
    // 坏文档指针会顺着 facts_page 的 documents[] 出去——那一路也要拦
    assert!(
