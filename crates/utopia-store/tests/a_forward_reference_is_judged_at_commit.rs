//! 同表自指的前向引用在**提交边界**上判（0070 §1 递延约束触发器）。
//!
//! `facts.supersedes`、`facts.from_statement_id`、`relation_types.inverse_of`、
//! `relation_types.sub_property_of` 都指着同表的行——恢复/批量装载时目标可能
//! 在本语句之后才落盘。行级 BEFORE 触发器在落地那一刻看不见后面的行；递延
//! 约束触发器（DEFERRABLE INITIALLY DEFERRED）在提交时重估，那时整批都在。
//!
//! 写入形状分三种，各自的墙不一样：
//!   - **多行 INSERT / COPY**：一条语句一批行——FK 在语句末看整批（过），
//!     BEFORE 逐行看（看不见后面的目标）。递延约束是这里唯一的闸；
//!   - **顺序语句**：`supersedes`/`inverse_of`/`sub_property_of` 的 FK 是
//!     立即的——目标不存在的顺序写根本进不来，FK 先挡；
//!   - **replica 会话**（pg_restore --disable-triggers 的形状）：触发器与
//!     FK 全关——那一层没有任何触发器能管，接住它的是导出前的
//!     provenance_integrity 体检。
...
/// 顺序写装不出前向引用：supersedes 的 FK 是立即的，目标不存在的顺序写
/// 在第一条语句就被 FK 挡下——递延约束在这条路上是第二道墙
#[tokio::test]
async fn a_sequential_forward_reference_is_stopped_by_the_fk() -> anyhow::Result<()> {
    let Some(url) = utopia_store::test_db::url() else {
        return Ok(());
    };
    let pool = PgPool::connect(&url).await?;
    ...
    assert!(
        r.is_err(),
        "目标还不存在的 supersedes 顺序写，FK 在语句末就该报"
    );
