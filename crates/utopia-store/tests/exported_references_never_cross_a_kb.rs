//! 导出序列化触碰的**每一条**引用都不许跨库（0070 的完整版）。
//!
//! 出处链的三条边（evidence.chunk / evidence.document / chunk.document）只是
//! 一族兄弟边里最先长出来的：派生的前提、边上的属性、事实的主宾谓、
//! supersedes 与 from_statement_id、陈述来源、开放陈述的属性值、时间提及、
//! 类型与短语绑定、派生的规则、实体的类、类层级与 domain/range——同一条
//! 不变量下它们都会做出同一种伪造。落到别库的下场分两种：
...
async fn retained_row_validation_survives_dangling_and_late_state() -> anyhow::Result<()> {
    ...
    sqlx::query("INSERT INTO fact_evidence (fact_id, chunk_id, document_id) VALUES ($1, $2, $3)")
        ...
        .execute(&pool)
        .await?;
    utopia_store::export::evidence_page(&mut pool.begin().await?, f.a, None).await?;
    ...
    let err = utopia_store::export::evidence_page(&mut pool.begin().await?, f.a, None).await;
    let msg = format!("{err:?}");
